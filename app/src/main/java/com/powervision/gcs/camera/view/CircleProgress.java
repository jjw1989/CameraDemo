...
public class CircleProgress extends View {
    ...
    private float mSubCurProgress;                        // 子进度条当前值
    ...
    public CircleProgress(Context context, AttributeSet attrs) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.MyCircleProgressBar);

        mMaxProgress = array.getInteger(R.styleable.MyCircleProgressBar_Pb_max, DEFAULT_MAX_VALUE);                // 获取进度条最大值

        boolean bFill = array.getBoolean(R.styleable.MyCircleProgressBar_Pb_fill, DEFAULT_FILL_MODE);            // 获取填充模式
        int paintWidth = array.getInt(R.styleable.MyCircleProgressBar_Pb_Paint_Width, DEFAULT_PAINT_WIDTH);        // 获取画笔宽度
        mCircleAttribute.setFill(bFill);
        if (bFill == false) {
            mCircleAttribute.setPaintWidth(paintWidth);
        }

        int paintColor = array.getColor(R.styleable.MyCircleProgressBar_Pb_Paint_Color, DEFAULT_PAINT_COLOR);     // 获取画笔颜色

        Log.i("", "paintColor = " + Integer.toHexString(paintColor));
        mCircleAttribute.setPaintColor(paintColor);


        mCircleAttribute.mSidePaintInterval = array.getInt(R.styleable.MyCircleProgressBar_Pb_Inside_Interval, DEFAULT_INSIDE_VALUE);// 圆环缩进距离
     ...
     * 开启动画
     */
    public void startCartoom(float time) {
    ...
    class CartoomEngine {
        ...
        public synchronized void startCartoom(float time) {
