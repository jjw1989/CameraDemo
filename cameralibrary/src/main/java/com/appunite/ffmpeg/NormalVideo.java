...
public class NormalVideo {
    ...
    Context context;
   int vrmode=0;
    public NormalVideo(Context context,int vrmode){
        initVertexData();
    ...
    public void setShader(int vrmode) {
        if(this.vrmode==vrmode)
            return;
        switch (vrmode){
            case 1:{
            ...
            case 2: {
                String mVertexShader = ShaderUtil.loadFromAssetsFile("vertexShader.sh", context);
                String mFragmentShader = ShaderUtil.loadFromAssetsFile("fragShaderDistor.sh", context);
                mProgram = ShaderUtil.createProgram(mVertexShader, mFragmentShader);
                if (mProgram == 0) {
                    return;
                }
    ...
    private  NormalVideo sNormalVideo;

//    public  NormalVideo getInstance(Context context){
